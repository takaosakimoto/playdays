...
@Injectable()
export class SubscribeNewFriendRequestNotificationAction extends Action<Request, FriendRequest> {
  ...
  onExecute(r: Request) {
    ...
    const eventName = 'friend_request:created';
    this._notificationChannel.join(topic)
      .filter(o => o)
      .flatMap(o => this._notificationChannel.subscribe(topic, eventName))
      .subscribe(
        (rf: FriendRequest) => {
          console.debug('NotificationChannel friend request created', rf);
